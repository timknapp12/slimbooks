...
-- Create checks table for check printing (linked to transactions register)
CREATE TABLE checks (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    company_id UUID REFERENCES companies(id) ON DELETE CASCADE NOT NULL,
    user_id UUID REFERENCES users(id) ON DELETE SET NULL,
...
-- RLS Policies for checks (multi-company support)
...
CREATE INDEX idx_checks_transaction_id ON checks(transaction_id);
CREATE INDEX idx_checks_journal_entry_id ON checks(journal_entry_id);
CREATE INDEX idx_checks_payable_id ON checks(payable_id);
CREATE INDEX idx_checks_date ON checks(date);
CREATE INDEX idx_checks_status ON checks(status);
CREATE INDEX idx_checks_check_number ON checks(company_id, check_number);
