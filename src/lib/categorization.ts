...
export const autoCategorizeTranaction = (description: string, type: 'income' | 'expense' | 'asset' | 'liability' | 'equity'): string => {
  ...
  if (type === 'income') {
    ...
    if (desc.includes('refund') || desc.includes('return')) return 'Other Income'
    ...
    if (desc.includes('cash') || desc.includes('bank') || desc.includes('checking') || desc.includes('savings')) return 'Cash'
    ...
    if (desc.includes('equipment') || desc.includes('furniture') || desc.includes('computer')) return 'Equipment'
    ...
    if (desc.includes('accrued') || desc.includes('unpaid') || desc.includes('owed')) return 'Accounts Payable'
    if (desc.includes('tax') || desc.includes('irs') || desc.includes('withholding')) return 'Income Tax Payable'
    ...
    if (desc.includes('loan') || desc.includes('borrow') || desc.includes('repay')) return 'Owner\'s Capital'
    if (desc.includes('transfer') || desc.includes('move') || desc.includes('between')) return 'Owner\'s Capital'
    if (desc.includes('investment') || desc.includes('stock') || desc.includes('bond') || desc.includes('crypto')) return 'Owner\'s Capital'
...
export async function validateAndMapCategory(
  ...
  try {
    ...
    const { data: accounts, error } = await supabase
      ...
      .eq('is_active', true) as { data: ChartOfAccountsRow[] | null; error: PostgrestError | null }
    ...
    const exactMatch = accounts.find((acc: ChartOfAccountsRow) => acc.account_name === category)
    if (exactMatch) {
      return category
    }
    ...
    if (mappedCategory) {
      const mappedMatch = accounts.find((acc: ChartOfAccountsRow) => acc.account_name === mappedCategory)
      if (mappedMatch) {
    ...
    const typeMatch = accounts.find((acc: ChartOfAccountsRow) => acc.account_type === accountTypeMap[type])
    if (typeMatch) {
