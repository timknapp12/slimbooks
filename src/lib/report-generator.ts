...
export async function generateFinancialReports({
  ...
  transactions.forEach(
    (transaction: { type: string; category: string; amount: number }) => {
      ...
      if (transaction.type === 'income') {
        ...
        if (accountType === 'revenue') {
          revenueByCategory[transaction.category] =
            (revenueByCategory[transaction.category] || 0) + amount
          totalRevenue += amount
        } else {
          otherIncomeByCategory[transaction.category] =
            (otherIncomeByCategory[transaction.category] || 0) + amount
        ...
        if (accountType === 'expense') {
          ...
          const accountNumber =
            chartOfAccounts?.find(
              (acc: { account_name: string; account_number: string }) =>
                acc.account_name === transaction.category
            )?.account_number || '0'
          if (accountNumber >= '5000' && accountNumber < '6000') {
            costOfGoodsSoldByCategory[transaction.category] =
              (costOfGoodsSoldByCategory[transaction.category] || 0) + amount
            totalCostOfGoodsSold += amount
          } else {
            operatingExpensesByCategory[transaction.category] =
              (operatingExpensesByCategory[transaction.category] || 0) + amount
            totalOperatingExpenses += amount
          }
        } else {
          otherExpensesByCategory[transaction.category] =
            (otherExpensesByCategory[transaction.category] || 0) + amount
  ...
  transactions.forEach(
    (transaction: { type: string; category: string; amount: number }) => {
      ...
      if (accountType === 'asset') {
        assetsByCategory[transaction.category] =
          (assetsByCategory[transaction.category] || 0) + amount
        totalAssets += amount
      } else if (accountType === 'liability') {
        liabilitiesByCategory[transaction.category] =
          (liabilitiesByCategory[transaction.category] || 0) + amount
        totalLiabilities += amount
      } else if (accountType === 'equity') {
        equityByCategory[transaction.category] =
          (equityByCategory[transaction.category] || 0) + amount
        totalEquity += amount
      }
    }
  )
  ...
  const accountingEquationBalance =
    totalAssets - (totalLiabilities + totalEquity)
  if (Math.abs(accountingEquationBalance) > 0.01) {
    // Adjust equity to balance the equation
    equityByCategory['Retained Earnings'] =
      (equityByCategory['Retained Earnings'] || 0) + accountingEquationBalance
  ...
  transactions.forEach(
    (transaction: { type: string; category: string; amount: number }) => {
      ...
      if (accountType === 'revenue') {
        ...
        operatingActivitiesByCategory[transaction.category] =
          (operatingActivitiesByCategory[transaction.category] || 0) + amount
        ...
        if (accountNumber >= '1400' && accountNumber < '2000') {
          investingActivitiesByCategory[transaction.category] =
            (investingActivitiesByCategory[transaction.category] || 0) - amount
        ...
        financingActivitiesByCategory[transaction.category] =
          (financingActivitiesByCategory[transaction.category] || 0) + amount
  ...
  if (netIncome !== 0) {
    operatingActivitiesByCategory['Net Income'] =
      (operatingActivitiesByCategory['Net Income'] || 0) + netIncome
    ...
    chartOfAccounts?.map(
      (account: {
        ...
        const accountTransactions = transactions.filter(
          (t: { category: string }) => t.category === account.account_name
        )
        const debit = accountTransactions
          .filter((t: { type: string }) =>
            ['asset', 'expense'].includes(t.type)
          )
          .reduce(
        ...
        const credit = accountTransactions
          .filter((t: { type: string }) =>
            ['liability', 'equity', 'revenue'].includes(t.type)
          )
          .reduce(
            (sum: number, t: { amount: number }) => sum + Number(t.amount),
            0
  ...
  const accountTransactions: {
    [account: string]: Array<{ type: string; amount: number }>
  } = {}
  transactions.forEach(
    (transaction: { category: string; type: string; amount: number }) => {
