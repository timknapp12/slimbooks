...
async function parseTransactionsInChunks(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      const prompt = createTransactionParsingPrompt(chunks[i], availableCategories)
...
export async function parseTransactionsWithAI(
  ...
  try {
    ...
    if (pdfText.length > maxInputLength) {
      console.log(`PDF text is ${pdfText.length} chars, chunking for processing`)
      return await parseTransactionsInChunks(pdfText, availableCategories)
    }
...
function parseAIResponse(responseText: string): ParsedTransaction[] {
  try {
    ...
    console.log('AI Response last 200 chars:', responseText.substring(Math.max(0, responseText.length - 200)))
    ...
    if (jsonStart === -1 || jsonEnd === -1 || jsonStart >= jsonEnd) {
      console.warn('No valid JSON array found in response, returning empty array')
    ...
    console.log('Extracted JSON last 100 chars:', jsonString.substring(Math.max(0, jsonString.length - 100)))
    
    // Try to fix common JSON issues before parsing
    const fixedJsonString = fixCommonJsonIssues(jsonString)
    console.log('Fixed JSON length:', fixedJsonString.length)
    console.log('Fixed JSON differs from original:', fixedJsonString !== jsonString)
    ...
    try {
      ...
      console.error('Problematic JSON around position 8:', fixedJsonString.substring(0, 50))
    ...
    if (recoveredTransactions.length > 0) {
      console.log(`Recovered ${recoveredTransactions.length} transactions using fallback parsing`)
      ...
      if (jsonStart !== -1) {
        ...
        if (lastCompleteObject !== -1) {
          ...
          try {
            ...
            const secondLastObject = jsonString.lastIndexOf('}', lastCompleteObject - 1)
...
function validateTransactions(transactions: ParsedTransaction[]): ParsedTransaction[] {
  const validTransactions: ParsedTransaction[] = []
  
  for (const transaction of transactions) {
    try {
      // Validate required fields
      if (!transaction.date || !transaction.description || transaction.amount === undefined || !transaction.category) {
...
export function estimateParsingConfidence(
  ...
  const hasValidDates = transactions.every(t => !isNaN(new Date(t.date).getTime()))
  const hasReasonableAmounts = transactions.every(t => Math.abs(t.amount) > 0 && Math.abs(t.amount) < 1000000)
  const hasDescriptions = transactions.every(t => t.description && t.description.length > 2)
  
  if (hasIndicators && hasValidDates && hasReasonableAmounts && hasDescriptions && transactions.length >= 3) {
    return 'high'
  } else if (hasValidDates && hasReasonableAmounts && transactions.length >= 1) {
    return 'medium'
  } else {
    return 'low'
  }
